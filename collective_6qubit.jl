...
println("Number of Sectors K= ", size(P_C)[1]) 
        for i in 1:size(P_C)[1]
            println("d_$i = ", size(P[i])[1], "\t d'_$i = ", Int(round(real(tr(P[i][1])), digits=1)))
        end
# 306.628 s (4879240 allocations: 461.74 GiB)